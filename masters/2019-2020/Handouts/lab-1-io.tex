...
%The lab bundle will build two versions of the benchmark: \code{io-static}
%and \code{io-dynamic}.
...
%   benchmark by specifying the \code{O\_DIRECT} flag to the \code{open()}
%   system call on the target file.
%   When switching from buffered mode, the first measurement using \code{-d}
...
by instrumenting return from \code{execve()} and entry of \code{exit()}
for the program under test.

This run of \code{io-static}, reading the data file \code{/data/iofile},
...
time measurement, such as provided by \code{time}, may suffer from
...
\item[\url{/data/iofile}] A writable file in a journalled UFS filesystem on
  the SD card.
  You will need to create the file using \code{-c} before performing
  \code{read()} or \code{write()} benchmarks.
\end{description}

\section*{Jupyter}

\begin{aquote}{\url{jupyter.org}}
The Jupyter Notebook is a web application that allows you to create and share
documents that contain live code, equations, visualisations and explanatory
text. Uses include: data cleaning and transformation, numerical simulation,
statistical modelling, machine learning and much more.
\end{aquote}
...
% (or \code{io-dynamic} as required).
